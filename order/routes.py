...
from flask import Blueprint, Flask, request,jsonify,make_response,blueprints
import requests
from models import Order,OrderItem
import requests
order_bluprint = Blueprint('order_api_routes',__name__,url_prefix="/api/order")
...
@order_bluprint.route('/',methods=['GET'])
def get_open_order():
...
@order_bluprint.route('/all',methods=['GET'])
def all_orders():
...
@order_bluprint.route('/add-item',methods=['POST'])
def add_item():
    api_key = request.headers.get('Authorization')
    if not api_key:
        return jsonify({'message':'Not logged in'}), 401
...
@order_bluprint.route('/checkout',methods=['POST'])
def checkout():
    api_key = request.headers.get('Authorization')
    if not api_key:
        return jsonify({'message':'not logged in'}),401
